# This version generates lyrics using emotional information from MEXICA.
# http://www.mayaackerman.info/pub/ballad-automated.pdf
...
# Extract type of emotion (love competition or clash in emotion) between characters in story line.
# The files (ce_eg#.txt in corpus folder) are generated by MEXICA.
# This part of code creates tension_dictionary.
with open('./corpus/ce_eg5.txt') as tension_file:
    flag = True
    same_value = True
    val = []
...
# This function parses actions.txt containing emotional information and 
# converts it into dictionary for easy extraction of sentiment score between characters.
...
# This function returns last word in sentence.
...
# This function calls markov_lyrics file.
def getMarkovBatch():
...
# This function returns words rhyming with the input word. 
...
# It returns rhyme quality score between two words.
...
# Some rhyme pairs are given high score than others.
...
# Using NLTK, find words that rhyme with input word.
...
# PRE and POST are two types of emotional score generated by MEXICA. 
# PRE stands for before action.
# POST stands for after action.
# Here, PRE score is extracted from actions.
...
# PRE and POS scores are extracted from actions_list.
# actions_list is look up dictionary created from actions.text file.
# actions_list contains PRE POS scores for all possible actions generated by MEXICA.
# This function returns sentiment dictionary (PRE and POS scores) for a particular action from actions_list.
...
def get_sentiment_value(pattern, tension_dictionary):
    ...
    lc_found = False
    ce_found = False
    for key, value in tension_dictionary.items():
...
# This funtion uses a sentiment API to extract sentiments of candidate lines.
...
# This function takes the narrative line as input and generates a candidate line.
def get_rhyme(sentence):
    ...
    #  Get sentiment score of candidate lines
    closest_sentences = get_corpus_score(close_sentences_list)
    #  Get sentiment score of narrative line
    story_score = get_sentiment_value(pattern, tension_dictionary)
    rhyme_sentences = []
    # Match sentiments of narrtive line with candidate lines
    if story_score > 0:
...
# Our story contains subjects. So candidate lines need to be connected to them.
# Candidate lines are 
...
# This function reads story and for each line in story, it calls other functions to get a good poetic line.
# A good line is determined by its syllables count, rhyme quality and sentiment score.
